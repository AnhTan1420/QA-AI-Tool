...
export function GenerateWorkspace({ projectId }: { projectId: string }) {
  ...
  function exportCsv() {
    ...
    const rows = testCases.map((testCase) => [testCase.code, testCase.title, testCase.category, testCase.priority, testCase.final_expected_result]);
    ...
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      <section className="space-y-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            disabled={isPending || testCases.length === 0}
          ...
          <button disabled={testCases.length === 0} onClick={exportCsv} className="rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-800 hover:border-emerald-200 disabled:opacity-50">
            Export CSV
          </button>
        </div>
      </section>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black text-slate-950">{testCases.length} test cases</h2>
            </div>
            {review && (
          ...
          <div className="mt-6 space-y-5">
            ...
            {testCases.length === 0 && <div className="rounded-2xl border border-dashed border-slate-200 p-10 text-center text-slate-500">Chưa có test case. Hãy chạy Generate để bắt đầu.</div>}
          </div>
        </div>
