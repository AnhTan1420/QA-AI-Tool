...
export function ResultsPanel({ workspace }: { workspace: GenerateWorkspaceState }) {
  ...
  return (
    <div className="rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-[0_2px_20px_-4px_rgba(15,23,42,0.06)] backdrop-blur-sm">
      ...
      {workspace.isPending && (
        <div className="mt-4 animate-[fadeIn_0.2s_ease] rounded-2xl border border-blue-200 bg-blue-50/60 p-5">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-bold text-blue-800">
              {workspace.t.generateWorkspace.generatingSteps[workspace.generatingStep]}
            </p>
          </div>
          <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-blue-100">
            <div
              ...
              style={{
                width: `${Math.round(((workspace.generatingStep + 1) / workspace.t.generateWorkspace.generatingSteps.length) * 100)}%`,
              }}
            />
          </div>
          <p className="mt-2 text-xs font-medium text-blue-600/80">{workspace.t.generateWorkspace.generatingHint}</p>
        </div>
      )}
