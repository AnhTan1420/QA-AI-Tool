...
import { inspectEnvironment, runGeneratedScript } from '@/lib/automation/browser-runner';
...
import { playwrightScriptSchema, type EnvironmentConfig, type PageObject } from '@/lib/validators/playwright';
 ...
 * + /api/automation/run — see components/test-case/automation/use-automation.ts),
...
export async function processClaimedBatchItem(
  ...
  if (existingScript) {
    ...
    let elementMap;
    try {
      const inspected = await inspectEnvironment(environment, [], undefined);
