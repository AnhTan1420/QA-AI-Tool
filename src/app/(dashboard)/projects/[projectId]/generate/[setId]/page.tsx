...
import { getPriorityStyle } from '@/models/test-case-taxonomy';
import { BackLink } from '@/views/layout/back-link';
import { RequirementTraceabilitySection, type RequirementTraceabilityRow } from '@/views/test-case/requirement-traceability-section';
...
export default async function GenerateResultPage({ params }: { params: Promise<{ projectId: string; setId: string }> }) {
  ...
  const { data: traceabilityRows } = await supabase
    .from('requirement_traceability')
    .select('requirement_clause, is_covered, test_case_id')
    .eq('set_id', setId);

  // Gop cac dong requirement_traceability (1 dong / 1 lien ket clause<->case)
  // thanh 1 dong / clause voi danh sach day du case dang cover no - test case
  // code/title lay lai tu `testCases` da fetch o tren (khong query them lan
  // nua), khop qua test_case_id.
  const testCaseById = new Map((testCases ?? []).map((tc) => [tc.id, tc]));
  const traceabilityByClause = new Map<string, RequirementTraceabilityRow>();
  for (const row of traceabilityRows ?? []) {
    const existing: RequirementTraceabilityRow = traceabilityByClause.get(row.requirement_clause) ?? {
      clause: row.requirement_clause,
      isCovered: false,
      coveredBy: [],
    };
    if (row.is_covered) existing.isCovered = true;
    if (row.test_case_id) {
      const tc = testCaseById.get(row.test_case_id);
      if (tc) existing.coveredBy.push({ id: tc.id, code: tc.code, title: tc.title });
    }
    traceabilityByClause.set(row.requirement_clause, existing);
  }
  const traceability = Array.from(traceabilityByClause.values());
  ...
  return (
    <div className="space-y-6">
      ...
      <RequirementTraceabilitySection rows={traceability} projectId={projectId} t={t} />
