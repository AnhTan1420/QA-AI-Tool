...
export async function POST(req: Request) {
  try {
    ...
    // 1b) Resolve project_id + load Page Object Registry BEFORE calling the AI, so the
    // Codegen Agent can see existing Page Objects for this project and reuse/extend them
    // instead of recreating them (Automation Agent Rebuild §4.1). Best-effort: a supabase
    // client is needed regardless (also used below to save the script), and registry
    // loading NEVER blocks generation - an empty registry (or a lookup failure) just means
    // the "EXISTING PAGE OBJECT REGISTRY" prompt section is omitted, identical to today.
    const supabase = await createClient();
    const testCaseId = typeof rawBody?.test_case_id === 'string' ? rawBody.test_case_id : null;
    const projectId = input.project_id ?? (testCaseId ? await resolveProjectIdForTestCase(supabase, testCaseId) : null);
    ...
    // 3a) Registry Merge Engine (Phase 1 - pure/synchronous, see
    // page-object-registry-orchestrator.ts). MUST run before any check below that reads
    // page_objects[].code, and before saving automation_scripts - the AI was instructed to
    // emit only NEW methods for a page that already matches a registry entry, so
    // parsed.data.page_objects may contain incomplete (delta-only) classes at this point.
    // computeRegistryMergePlan replaces `code` with the full merged class wherever a match
    // was found, so every downstream check/save sees complete, runnable code. Only runs
    // when we have both a project and a test case to attribute the change to - an ad-hoc/
    // preview call (no saved test case yet) skips registry integration entirely, same as
    // today's behavior.
    ...
    // Runs AFTER the registry merge (3a) so a goto() that lives only in an already-merged
    // registry entry (not re-emitted by the AI on this call) is correctly seen here too.
    ...
    // Also runs against the post-merge code (3a) for the same reason as 3c.
    ...
    // 3e) Surface the AI's own self-reported registry conflicts (see OUTPUT CONTRACT #6 in
    // the prompt) as warnings too - these are a proactive signal from the model itself,
    // separate from (and in addition to) the Merge Engine's own deterministic detection
    // in 3a/4 below, which is what actually gates the registry write.
    ...
    // 4) savedScriptId đã lưu ở bước trên (test_case_id nếu client gửi kèm) - giữ optional
    // để route này vẫn dùng được độc lập (VD preview trước khi có test case đã lưu).
    ...
    if (testCaseId) {
      ...
      if (saveError) {
        ...
        // Phase 2 of the Registry Merge (see page-object-registry-orchestrator.ts) - runs
        // AFTER the script row exists (needs its id for automation_script_page_object_refs).
        // Best-effort: never throws, and its notices are appended to BOTH the just-saved
        // row's `warnings` column (so reopening this script later still shows them) and the
        // response payload below (`parsed.data.warnings`) for immediate visibility.
        if (mergePlan) {
