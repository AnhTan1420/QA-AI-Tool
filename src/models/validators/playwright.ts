...
// ── Execution mode (Automation Agent Rebuild — see docs/automation-agent-rebuild.md) ──
// 'serverless' = existing eval-based runner (lib/automation/browser-runner.ts#runGeneratedScript),
//   renamed in the UI to "Preview" - fast, works on Vercel Hobby, but is NOT a
//   real @playwright/test execution (no trace/video/retry/report).
// 'self_hosted' = real `npx playwright test` via lib/automation/playwright-test-runner.ts,
//   only actually runnable when AUTOMATION_RUNTIME=local - see assertExecutionModeAllowed
//   below for the same server-side enforcement pattern assertBrowserAllowed already uses.
 ...
 * could still submit execution_mode: 'self_hosted' on a Vercel deployment (stale UI,
...
export const environmentConfigSchema = z
  .object({
    ...
    login: z
      ...
      .optional(),
    execution_mode: executionModeSchema.default('serverless'),
  })
  ...
  .refine((v) => {
    try {
      assertExecutionModeAllowed(v.execution_mode);
      return true;
    } catch {
      return false;
    }
  }, { message: 'Chế độ "Full run (self-hosted)" chỉ khả dụng khi tự host QAJD (AUTOMATION_RUNTIME=local).' });
...
// ── Page Object Registry (Automation Agent Rebuild) ─────────────────────────
// A registry entry is the project-scoped, incrementally-extended source of truth
// for a single Page Object class - see docs/automation-agent-rebuild.md §4.1.
// automation_scripts.page_objects (below) remains a per-generation SNAPSHOT used
// to run/replay that exact reviewed version; this is the separate, longer-lived
// entity the Codegen Agent reads for context and the Merge Engine writes to.
// Declared BEFORE playwrightScriptSchema because it references registryConflictSchema.
export const methodSignatureSchema = z.object({
  name: z.string().min(1),
  params: z.string().default(''), // raw param-list text for display only, not parsed
  added_by_test_case_id: z.string().uuid().nullable().default(null),
...
export const registryEntrySchema = z.object({
  ...
  page_url_pattern: z.string().nullable().default(null),
  code: z.string().min(1),
  method_signatures: z.array(methodSignatureSchema).default([]),
  version: z.number().int().positive().default(1),
});
export type RegistryEntry = z.infer<typeof registryEntrySchema>;

// Slimmed-down shape actually injected into the codegen prompt (see
// buildPlaywrightCodegenPrompt's "EXISTING PAGE OBJECT REGISTRY" section) - full
// `code` is included so the model can extend it correctly, but callers build this
// from a RegistryEntry rather than re-querying, so it stays 1:1 with what's stored.
export const registryContextEntrySchema = registryEntrySchema.pick({
  class_name: true,
  file_name: true,
  page_label: true,
  page_url_pattern: true,
  code: true,
  method_signatures: true,
});
export type RegistryContextEntry = z.infer<typeof registryContextEntrySchema>;

// A registry conflict = the Codegen Agent proposed a method with the SAME NAME as
// one already in the registry, but a DIFFERENT body. Never auto-resolved (see P3,
// docs/automation-agent-rebuild.md) - always queued for a human to reconcile.
...
export const playwrightScriptSchema = z.object({
  ...
  // Populated by the model ONLY when a registry context was given (see
  // registryContextEntrySchema) and a step needed an existing method changed - the
  // model reports the discrepancy instead of silently editing it. Optional/defaulted
  // so this stays backward-compatible with a first-time generation (no registry yet).
  registry_conflicts: z.array(registryConflictSchema).default([]),
});
...
export const playwrightCodegenRequestSchema = z.object({
  ...
  // Optional: needed to load Page Object Registry context (see registryContextEntrySchema)
  // for this project before calling the AI, and to reconcile the AI's proposed page
  // objects back into that registry after. Optional so this route stays usable without
  // a project context (e.g. an ad-hoc preview call) - registry integration is simply
  // skipped when absent, never a hard requirement.
...
export const playwrightHealRequestSchema = z.object({
  ...
  project_id: z.string().uuid().optional(), // see playwrightCodegenRequestSchema — same optional registry wiring
});
export type PlaywrightHealRequest = z.infer<typeof playwrightHealRequestSchema>;

// 'flaky' = self-hosted only (see P4, docs/automation-agent-rebuild.md): first
// attempt failed, an automatic retry then passed. Reported distinctly from
// 'passed' — never silently folded into a green result.
export const automationRunStatusSchema = z.enum(['passed', 'failed', 'error', 'flaky']);
...
export const automationRunResultSchema = z.object({
  ...
  // Self-hosted ("Full run") only — populated by lib/automation/playwright-test-runner.ts.
  // Always undefined for a serverless Preview run.
  attempts: z.number().int().positive().default(1),
  trace_path: z.string().optional(), // local tmp path before upload, mirrors screenshot_path
  video_path: z.string().optional(),
  html_report_path: z.string().optional(), // local tmp dir (zipped before upload)
  execution_mode: executionModeSchema.default('serverless'),
});
