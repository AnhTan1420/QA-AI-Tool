...
export async function processClaimedBatchItem(
  ...
  if (existingScript) {
    ...
    try {
      ...
      // /api/ai/playwright/route.ts: resolve project + load registry BEFORE the AI call
      // so the prompt can offer reuse, then reconcile AFTER the script is saved. An extra
      // query per batch item vs. joining test_case_sets inline in the earlier select —
      // accepted for now to reuse ONE canonical resolver rather than duplicate the join.
      const projectId = await resolveProjectIdForTestCase(supabase, item.test_case_id);
      ...
      const promptEnvironment = {
        ...
        auth_mode: environment.cookie_token ? ('cookie' as const) : environment.login ? ('login' as const) : ('none' as const),
      ...
      // Registry Merge Engine, Phase 1 (pure/synchronous) — MUST run before any check
      // below that reads page_objects[].code and before saving, exactly like
      // /api/ai/playwright/route.ts: replaces delta-only code from a registry-matched
      // page with the full merged class so the saved snapshot is always runnable.
      const mergePlan = projectId ? computeRegistryMergePlan(registry, parsed.data.page_objects, item.test_case_id) : null;
      if (mergePlan) {
      ...
      // otherwise-successful batch item.
      if (mergePlan && projectId) {
        await applyRegistryMergePlan(supabase, { projectId, testCaseId: item.test_case_id, scriptId: saved.id, plan: mergePlan });
      }
    } catch (err: any) {
